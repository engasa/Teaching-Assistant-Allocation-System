...
public class UserInterface {
	...
	public final static void main(String[] args) throws IOException, ParseException {
		...
		case "addcourse":
			...
			System.out.println("The course/s with the given key " + args[2] + " has been added");
			System.out.println("course/s can be found under the taschd_dir\\courses and taschd_dir\\terms\\"+args[3]+"\\courses folder");
			break;
		...
		case "addca":
			...
			// int caID = args[2] (Ex: 123432,234233,432212)
			// String term = args[3] (Ex: 2188)
	        ...
	        String[] cred = {userId, new String(passwordArray), new String(passcodeArray) };
	        String[] caid = args[2].split(",");
	        int caidLen = caid.length;
	        int[] caArray = null;
	        for(int i=0; i< caidLen; i++){
	        	caArray[i] = Integer.parseInt(caid[i]);
	        }
			
			ui.getCAFromWebReg(args[1], cred, caArray, args[3]);
			System.out.println("The CA Schedule/s with the given CAID " + args[2] + " has been added");
			System.out.println("CA Schedule/s can be found under the taschd_dir\\ca_pool and taschd_dir\\terms\\"+args[3]+"\\assistants folder");
			break;
